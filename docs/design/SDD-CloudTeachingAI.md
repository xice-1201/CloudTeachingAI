...
>
> v2.2 修订：删除 Docker Compose 服务清单中重复的消息层条目；在第 5.5 节补充 analysis-agent API 端点；在 assign-db ER 图中补充 `OUTBOX_MESSAGE` 表；在索引规划中补充 `outbox_message` 和 `analysis_report` 表索引；在 Kafka Topic 规划中补充 `submission.reviewed` 事件；补充 `X-Internal-Token` 轮换机制说明；明确上传状态丢失时客户端的处理逻辑；补充能力测试题目来源说明；优化 notify-service WebSocket 多实例路由为定向推送，减少广播冗余。
>
> v2.3 修订：补充 analysis-agent HPA 的 Kafka 消费延迟触发条件；在 QUESTION 表索引中补充 is_active 字段；在 SUBMISSION 表补充 student_id 索引；在 RESOURCE_KNOWLEDGE 表补充 source 字段并更新 tag-agent 降级说明；将 chat-agent 上下文管理从固定 20 条改为按 Token 数动态截断；补充 WebSocket JWT Token 日志脱敏建议；补充 X-Internal-Token 动态化进阶方案；明确 NotificationBell 降级轮询间隔为 30 秒；补充视频进度 beforeunload 强制上报；删除第 7.6 节重复 API 端点列表，改为交叉引用；补充 notify-service WebSocket Session Affinity 配置说明；补充 media-service 上传状态持久化评估说明。
>
...
CloudTeachingAI采用**微服务分布式架构**，系统分为五层：客户端层、接入层、微服务层、消息层、数据层。共包含**12个微服务**（7个Java业务服务 + 5个Python AI智能体服务）。

**架构特点**：
- **服务独立**：各服务独立开发、部署、扩展
- **异步解耦**：通过Kafka事件总线实现服务间松耦合
- **数据隔离**：每服务独立数据库，避免耦合
- **AI赋能**：5个AI智能体贯穿教学全流程
- **高可用**：多副本部署，主从复制，自动故障转移

### 0.2 架构分层说明

| 层次 | 组件 | 职责 | 技术栈 |
|------|------|------|--------|
| **客户端层** | Vue 3 SPA | 用户界面渲染、交互、状态管理 | Vue 3 + TypeScript + Vite |
| **接入层** | API Gateway + CDN | 统一入口、HTTPS终止、JWT鉴权、路由转发、限流 | Spring Cloud Gateway + CDN |
| **服务治理** | Nacos | 服务注册与发现、健康检查、分布式配置中心 | Nacos 2.3+ |
| **微服务层** | 12个微服务 | 业务逻辑处理，各服务职责单一，独立部署 | Spring Boot 3.3 + FastAPI |
| **消息层** | Apache Kafka | 服务间异步解耦，事件驱动，保证消息可靠投递 | Kafka 3.7+ |
| **数据层** | 数据库 + 缓存 + 存储 | 数据持久化，按服务边界隔离 | PostgreSQL 16 + Redis 7 + MinIO |
...
| 服务 | 端口 | 核心职责 | 数据库 |
|------|------|---------|--------|
| auth-service | 8001 | 登录、JWT签发与刷新、密码重置 | auth-db |
| user-service | 8002 | 用户账号CRUD、角色管理、导师关系 | user-db |
...
| notify-service | 8006 | 站内通知、WebSocket推送、邮件发送 | notify-db |
| media-service | 8007 | 文件上传（tus）、视频转码、预签名URL | — |

**AI智能体服务（Python技术栈）**

| 服务 | 端口 | 核心职责 | 数据库 |
|------|------|---------|--------|
| tag-agent | 8101 | AI分析资源内容，建议知识点标签，构建知识图谱 | vector-db |
| nav-agent | 8102 | 基于能力图谱生成个性化学习路线 | vector-db |
| grade-agent | 8103 | AI自动批改作业，生成评分和评语 | — |
| chat-agent | 8104 | AI智能助手答疑（SSE流式响应） | chat-db |
| analysis-agent | 8105 | 账号健康度分析、平台使用分析、内容合规预审 | analysis-db |
...
> **讨论区归属**：课程讨论区（帖子、回复、公告）作为 course-service 的扩展功能实现，不单独拆分服务。讨论区数据存储在 course-db，新增 `DISCUSSION_POST` 和 `DISCUSSION_REPLY` 表。帖子内容在 course-service 后端使用白名单过滤（Spring HtmlUtils）防止 XSS，管理员下架操作通过 course-service 提供的管理接口执行。
...
| `submission.reviewed` | assign-service | notify-service | 教师人工复核完成，通知学生成绩更新 |
| `login.event` | auth-service | analysis-agent | 用户登录行为（含登录时间、IP、失败次数），供账号健康度分析消费 |
| `gateway.access` | API Gateway | analysis-agent | Gateway 访问日志（路径、用户 ID、响应状态码），供平台使用分析消费 |
...
routes:
  ...
  - id: media-service
    ...
    filters: [JwtAuthFilter, RequestSizeFilter=2GB, StripPrefix=0]
...
> **安全规范**：Gateway 完成鉴权后，必须在转发请求前通过 `RemoveRequestParameter=token` 过滤器从 URL 中移除 `token` 参数，防止 Token 出现在下游服务访问日志中。此为强制要求，不依赖下游服务的日志脱敏配置。

> **文件上传大小限制**：media-service 路由配置 `RequestSizeFilter=2GB`（视频上传最大值）作为 Gateway 层硬限制，超限请求直接返回 413。media-service 内部根据文件类型进一步校验：视频 ≤ 2GB、文档/课件 ≤ 200MB（PPT/PPTX）或 ≤ 100MB（其他文档）、作业附件 ≤ 50MB，不符合则返回错误码 40003。

> **Session Affinity**：notify-service 的 K8s Service 需配置 `sessionAffinity: ClientIP`（或使用 Nginx Ingress 的 sticky session），确保同一用户的 WebSocket 连接始终路由到同一实例，避免连接被路由到未持有该连接的实例。
...
erDiagram
    ...
    RESOURCE_KNOWLEDGE {
        ...
        enum source "FULL_CONTENT/TITLE_ONLY"
    }
    ENROLLMENT {
        bigint id PK
        bigint student_id
...
#### vector-db（tag-agent / nav-agent）

```mermaid
erDiagram
    KNOWLEDGE_POINT_EMBEDDING {
        int knowledge_point_id PK
        varchar knowledge_point_name
        vector embedding "1536 维，text-embedding-3-small 生成"
        timestamp updated_at
    }
    RESOURCE_EMBEDDING {
        bigint resource_id PK
        varchar resource_title
        text content_summary "提取文本的前 500 字"
        vector embedding "1536 维，text-embedding-3-small 生成"
        timestamp updated_at
    }
```
...
| course-db | enrollment | UNIQUE(student_id, course_id)（该唯一约束已覆盖"查询某学生选了哪些课"的高频查询，无需额外添加单列索引） |
| learn-db | ability_map | UNIQUE(student_id, knowledge_point_id) |
| learn-db | learning_progress | UNIQUE(student_id, resource_id) |
| learn-db | question | INDEX(knowledge_point_id, difficulty, is_active)，INDEX(created_at)（支持管理员按时间查询导入批次） |
| learn-db | outbox_message | INDEX(sent, created_at) |
| assign-db | submission | INDEX(assignment_id, status)，UNIQUE(assignment_id, student_id)，INDEX(student_id, submitted_at) |
| assign-db | outbox_message | INDEX(sent, created_at) |
| course-db | outbox_message | INDEX(sent, created_at) |
| notify-db | notification | INDEX(user_id, is_read, created_at) |
| analysis-db | analysis_report | INDEX(report_type, status)，INDEX(triggered_by, created_at) |
...
| vector-db | knowledge_point_embedding | UNIQUE(knowledge_point_id)，IVFFlat INDEX(embedding vector_cosine_ops, lists=100) |
| vector-db | resource_embedding | UNIQUE(resource_id)，IVFFlat INDEX(embedding vector_cosine_ops, lists=100) |
...
DELETE /api/v1/courses/{id}/enroll             # 学生退课
...
POST   /api/v1/admin/questions/batch-import     # 管理员批量导入题目（CSV）
POST   /api/v1/questions                        # 教师手动录入题目
PATCH  /api/v1/questions/{id}                   # 修改题目
DELETE /api/v1/questions/{id}                   # 停用题目（is_active=false）
```
...
> **多副本防重复转码**：media-service 在 K8s 下有多个副本，同一 `resource_id` 可能被多个实例同时触发转码。Worker 线程池在启动转码任务前须先获取 Redis 分布式锁 `transcode_lock:{resource_id}`（TTL 10 分钟，详见第 13.5 节），获取失败则跳过，由持有锁的实例负责完成转码。
...
> 上传状态仅存储在 Redis（TTL 24 小时），Redis 重启后进行中的上传记录会丢失，客户端需重新调用 `/uploads/init` 初始化新的上传任务。已完成上传并写入 MinIO 的文件不受影响。客户端在每次分片上传前应先发送 `HEAD /uploads/{upload_id}` 查询当前进度；若服务端返回 404，说明上传状态已丢失，tus-js-client 应捕获该错误并引导用户重新发起上传（调用 `/uploads/init`）。

> **可靠性评估**：当前设计以 Redis TTL 为主，适合中小规模场景。对于需要更高可靠性的场景（如企业级部署），可考虑将上传元数据（upload_id、offset、storage_key、resource_id）额外持久化到独立数据库，以避免 Redis 重启导致进行中的上传状态丢失。

### 6.3 能力图谱测试（learn-service）

**自适应测试引擎**

题目存储在 learn-db 的 `questions` 表，含字段：`knowledge_point_id`、`difficulty`（1–5）、`content`、`options`、`answer`。题目来源有两种途径：管理员通过后台批量导入（CSV 格式），或教师在课程管理页面针对具体知识点手动录入；两种途径均通过 learn-service 提供的内部管理接口写入，不对学生开放。
...
**题库不足降级处理**：测试开始前，learn-service 检查所选知识点的题库数量。若某知识点可用题目（`is_active=true`）不足 3 题，则跳过该知识点的测试，在测试结果中将其掌握程度标记为 `NONE`（未测试），并在前端提示"部分知识点题库尚未完善，已跳过，建议管理员补充题目"。若所有选定知识点均不足 3 题，则拒绝开始测试，返回错误提示。

测试进度保存在 Redis（`test_session:{test_id}`，TTL 7 天），支持断点继续。测试完成后发布 `ability.updated` 事件到 Kafka，触发 nav-agent 重算路线。

> **Redis 可靠性评估**：测试进度仅存于 Redis，Redis 重启后进行中的测试会话将丢失，学生需重新开始测试。对于需要更高可靠性的场景，可考虑将 `session_state` 同步持久化到 `ABILITY_TEST` 表的 `session_state` 字段（jsonb），Redis 作为读缓存，DB 作为兜底。若启用此方案，同步时机为：每道题答完后立即将最新 `session_state` 写入 DB（与 Redis 同步更新），恢复时优先读 Redis，Redis 不存在则从 DB 加载并重建 Redis 缓存。当前设计以 Redis 为主，适合中小规模场景。
...
- 查询 `WS_SESSION` 表获取目标用户所在的 `server_instance_id`；若与当前实例匹配，直接推送；若不匹配，通过 Redis Pub/Sub 定向发布到 `notify:{server_instance_id}` 频道，由持有该用户连接的实例负责推送，避免全实例广播
- 用户离线时，下次登录拉取未读通知列表
...
**降级处理**：无字幕或文件无法解析时，降级为基于标题和简介分析，将 `source` 字段标记为 `TITLE_ONLY`，界面提示"内容解析受限，建议手动补充标签"，方便教师识别低置信度标注。
...
输入：
  ...
  - 候选资源列表（向量检索 + Feign 调用 course-service，见下方说明）

处理：
  1. 筛选掌握程度 < MASTERED 的知识点
  2. 向量检索（主路径）：对每个薄弱知识点，在 vector-db 中用知识点嵌入向量做余弦相似度检索，
...
**触发**：管理员手动触发（按需生成）；持续消费 Kafka `user.behavior` 事件和 `login.event` 事件积累数据。

**分析场景**：

| 场景 | 输入数据 | 输出 |
|------|---------|------|
| 账号健康度评估 | 用户登录频率、活跃度、异常登录记录（来自 `login.event`）、学习行为数据（来自 `user.behavior`） | 账号健康度报告，标记异常账号 |
| 平台使用分析 | API Gateway 访问日志（由 Gateway 写入 Kafka `gateway.access` Topic，analysis-agent 消费）、`user.behavior` 事件（功能操作路径） | 功能接受度分析、隐藏需求挖掘、优化建议 |
| 内容合规预审 | 新上传课程资料文本内容 | 合规风险评估，标记疑似违规内容（召回率 ≥ 90%，误判率 ≤ 5%） |

> **平台使用分析数据来源**：功能模块访问频率等数据来自 API Gateway 的访问日志，而非业务服务。Gateway 在全局过滤器中将请求路径、用户 ID、响应状态码等结构化日志发布到 Kafka `gateway.access` Topic，analysis-agent 消费后聚合分析。`gateway.access` Topic 仅 analysis-agent 消费，不影响其他服务。
...
管理员通过 `/admin/stats` 路由查看报告，对应 analysis-agent 提供的 API 端点详见第 5.5 节 analysis-agent 部分。
...
- 微服务间 Feign 调用携带内部服务凭证头 `X-Internal-Token`，防止绕过 Gateway 直接调用内部服务；`X-Internal-Token` 为静态共享密钥，通过 K8s Secret 以环境变量形式注入各服务，不写入代码或配置文件；需要轮换时，更新 K8s Secret 并滚动重启各服务（零停机）。进阶方案可采用 HMAC-SHA256(secret + timestamp/300) 生成短期动态 Token（5 分钟窗口），降低静态密钥泄露风险，各服务验证时允许前后一个时间窗口的偏差。
...
| notify-service | 2 | 4 | WebSocket 连接数 > 200/副本（通过 Prometheus Custom Metrics API 暴露，notify-service 使用 Micrometer 注册 `websocket.active.connections` 指标，HPA 通过 `custom.metrics.k8s.io` 读取） |
| media-service | 2 | 4 | CPU > 60% 或内存 > 70% |
| tag-agent | 1 | 3 | Kafka 消费延迟 > 30s |
| nav-agent | 1 | 3 | Kafka 消费延迟 > 30s |
| grade-agent | 1 | 4 | Kafka 消费延迟 > 60s |
| chat-agent | 2 | 6 | CPU > 60% 或活跃 SSE 连接数 > 100/副本（同样通过 Prometheus Custom Metrics API 暴露） |
| analysis-agent | 1 | 2 | 手动触发时扩容；Kafka `user.behavior` 消费延迟 > 120s 时自动扩容 |
...
| `NotificationBell` | 导航栏通知图标，WebSocket 实时更新未读数，断线自动降级轮询（间隔 30 秒） |
...
- 监听 `beforeunload` 事件，页面关闭前强制上报一次当前进度，防止最后 10 秒内的进度丢失
...
每条 Kafka 消息携带唯一 `event_id`（UUID），消费者处理前检查 Redis 中是否已处理过该 `event_id`（TTL 7 天，与 Kafka 消息保留期对齐，防止消费者宕机超过 24 小时后重启时重放历史消息导致重复处理），防止重复消费：

```
Redis Key: event_processed:{event_id}
处理逻辑：
  1. SETNX event_processed:{event_id} 1 EX 604800
  2. 返回 1（首次）→ 正常处理
...
> **多副本并发安全**：多个服务副本同时轮询 `outbox_message` 表时，使用 `SELECT ... FOR UPDATE SKIP LOCKED` 确保每条消息只被一个实例处理，其他实例跳过已被锁定的行，避免重复投递。

**Outbox 模式适用范围**：凡是"写 DB + 发 Kafka"需要原子性保证的服务，均应实现 Outbox 模式。当前覆盖范围：

| 服务 | 触发场景 | Outbox 表所在数据库 |
|------|---------|-------------------|
| assign-service | 作业提交 → `submission.created` | assign-db |
| learn-service | 能力图谱更新 → `ability.updated` | learn-db |
| course-service | 资源标注完成 → `resource.tagged`；知识点/资源/课程变更冗余同步事件 | course-db |

其他服务（如 auth-service 发布 `login.event`、notify-service 写通知后推送）对原子性要求较低（失败可接受重试或丢弃），暂不强制要求 Outbox 模式，但需在各服务内部做好异常日志记录。
