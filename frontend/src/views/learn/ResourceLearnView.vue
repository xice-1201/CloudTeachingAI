  ...
  <div class="learn-page" v-loading="loading">
    ...
    <div class="learn-body">
      <div v-if="resource?.type === 'VIDEO'" class="video-area">
        <video ref="videoEl" class="video-player" controls preload="metadata" playsinline @timeupdate="handleTimeUpdate" @ended="handleEnded" />
      </div>
...
const sidebarText = computed(() => (canTrackProgress.value ? `${progressPct.value}% 已完成` : '当前为教师预览模式，不记录学习进度。'))

let saveTimer: ReturnType<typeof setInterval> | null = null
let temporaryResourceUrl = ''

function resolveResourceUrl(url: string) { if (/^https?:\/\//i.test(url)) return url; if (url.startsWith('/')) return url; return `/${url}` }
function revokeTemporaryResourceUrl() { if (temporaryResourceUrl) { URL.revokeObjectURL(temporaryResourceUrl); temporaryResourceUrl = '' } }
function isInternalResourceUrl(url: string) { return url.startsWith('/api/') }
function getCourseId() { return Number(route.params.courseId) }

async function buildAuthorizedResourceUrl(url: string) {
  const token = localStorage.getItem('token')
  const response = await fetch(resolveResourceUrl(url), { headers: token ? { Authorization: `Bearer ${token}` } : {} })
...
async function handleEnded() {
  ...
  progress.value = await learnApi.updateProgress(String(resource.value.id), { courseId: getCourseId(), progress: 1, lastPosition: videoEl.value ? Math.floor(videoEl.value.currentTime) : undefined })
}
...
async function saveProgress() {
  ...
  progress.value = await learnApi.updateProgress(String(resource.value.id), { courseId: getCourseId(), progress: percent, lastPosition: isVideo && videoEl.value ? Math.floor(videoEl.value.currentTime) : undefined })
}
...
async function loadResourcePage() {
  ...
  if (!resourceData) { resourceUrl.value = ''; return }
  const resolvedUrl = resolveResourceUrl(resourceData.url)
  if (isInternalResourceUrl(resolvedUrl)) resourceUrl.value = await buildAuthorizedResourceUrl(resolvedUrl)
  else { revokeTemporaryResourceUrl(); resourceUrl.value = resolvedUrl }
  await nextTick()
  ...
  if (canTrackProgress.value && !progressData?.completed) progress.value = await learnApi.updateProgress(String(resourceData.id), { courseId: getCourseId(), progress: 1 })
}

function handleBeforeUnload() { if (canTrackProgress.value) saveProgress().catch(() => undefined) }
function downloadResource() { if (!resourceUrl.value || !resource.value) return; const link = document.createElement('a'); link.href = resourceUrl.value; link.download = resource.value.title; link.target = '_blank'; link.rel = 'noopener'; link.click() }
function restoreVideoPosition(player: HTMLVideoElement, lastPosition?: number) { if (!lastPosition || lastPosition <= 0) return; const applyPosition = () => { if (Number.isFinite(player.duration) && player.duration > 0) { player.currentTime = Math.min(lastPosition, Math.max(player.duration - 1, 0)); return } player.currentTime = lastPosition }; if (player.readyState >= 1) { applyPosition(); return } player.addEventListener('loadedmetadata', applyPosition, { once: true }) }
...
onMounted(async () => {
  ...
  try {
    ...
    if (canTrackProgress.value) saveTimer = setInterval(() => { saveProgress().catch(() => undefined) }, 10000)
...
.learn-header { height: 56px; background: #16213e; display: flex; align-items: center; padding: 0 20px; gap: 16px; color: #fff; justify-content: space-between; }
.learn-header-main { display: flex; align-items: center; gap: 16px; min-width: 0; }
...
.learn-sidebar { width: 300px; background: #fff; padding: 16px; overflow-y: auto; display: flex; flex-direction: column; gap: 16px; }
.progress-text,.resource-description { color: #909399; font-size: 13px; line-height: 1.6; }
.resource-tags { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 12px; }
.outline-chapter + .outline-chapter { margin-top: 16px; }
.outline-chapter-title { font-size: 13px; font-weight: 600; color: #303133; margin-bottom: 8px; }
.outline-resource { padding: 8px 10px; border-radius: 6px; cursor: pointer; color: #606266; font-size: 13px; }
.outline-resource:hover,.outline-resource.active { background: #ecf5ff; color: #409eff; }
</style>
